...
try {
  const [rows] = await pool.query('SELECT 1 as test, DATABASE() as current_db');
  ...
  console.error('  2. Database "Exam_DB_Cursor" exists');
  console.error('  3. User credentials in .env are correct');
  console.error('  4. Run: mysql -u root -p < src/db/schema.sql\n');
  await pool.end();
  process.exit(1);
}
